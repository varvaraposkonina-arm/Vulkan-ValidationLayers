...
TEST_F(VkLayerTest, TonyImageLayout) {
    if (!AddSurfaceInstanceExtension()) {
    ...
    VkFence fence;
    VkResult err = vk::CreateFence(m_device->device(), &fenceci, nullptr, &fence);
    ASSERT_VK_SUCCESS(err);
    PFN_vkAcquireNextImageKHR fpAcquireNextImageKHR =
        (PFN_vkAcquireNextImageKHR)vk::GetDeviceProcAddr(m_device->handle(), "vkAcquireNextImageKHR");
    VkResult ret = fpAcquireNextImageKHR(m_device->handle(), m_swapchain, MAXINT32, VK_NULL_HANDLE, fence, &image_index);
    VkAttachmentDescription attach[] = {
    ...
    err = vk::CreateRenderPass(m_device->device(), &rpci, nullptr, &rp1);
    ASSERT_VK_SUCCESS(err);
    attach[0].initialLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    err = vk::CreateRenderPass(m_device->device(), &rpci, nullptr, &rp2);
    VkImageViewCreateInfo ivci = {
        VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
        nullptr,
        0,
    ...
    err = vk::CreateFramebuffer(m_device->device(), &fci, nullptr, &fb1);
    fci.renderPass = rp2;
    err = vk::CreateFramebuffer(m_device->device(), &fci, nullptr, &fb2);
    ASSERT_VK_SUCCESS(err);
